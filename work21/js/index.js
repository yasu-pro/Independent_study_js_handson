...
const toggleSortById = (userData, sortIcon) => {
    ...
    if (sortIcon.classList.contains('both')) {
        ...
        const sortedData = sortById(userData, 'asc');
        ...
        const sortedData = sortById(userData, 'desc');
        updateTableBody(tableElem, sortedData);
    } else {
...
const setSortIconClickListener = () => {
    ...
    sortIcon.addEventListener('click', async () => {
        try {
            const userData = await fetchUser();
            toggleSortById(userData, sortIcon);
        } catch (error) {
